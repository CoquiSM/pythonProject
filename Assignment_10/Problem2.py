...
while choice.lower() !=  'd':
    choice = input("\nChoose a following operation: ( A ): Add || ( B ): Remove || ( C ): View Contact | ( D ): Stop | ( E ): Save to File ")
    if choice.lower() == "a": ##addition
        ...
        contactBookSTR = json.dumps(contactbook)
        fileSave = open("sample.txt", "w")
        fileSave.write(contactBookSTR)
        fileSave.close()
        print(fileSave)
